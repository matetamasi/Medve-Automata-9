...
Where it is not stated otherwise, let the alphabet be $Sigma = {a, b}$.
+ Construct a deterministic finite automaton for each of the following languages:
  #subtasks((
    [words containing exactly 3 letters],
    [words starting with letter $a$],
    [words containing only the letter $a$],
    [words ending on letter $b$],
    [words containing exactly 3 letters of $a$],
    [words not containing the letter $a$ $Sigma = {a,b,c}$],
    [words containing at least 3 letters of $a$],
    [words containing at least 3 letters of $a$ *and* containing at least 3 letters of $b$]
  ), cg: 1.8cm)
+ What laguages do the following automata accept? (Similarly to excercise 1, try constructing a rule that describes exactly which words the automata accepts.)
  #subtasks((
    [#maut(
      (
        S: (S:"a", A:"b"),
...
+ Construct a deterministic finite automaton for each of the following languages:
  #subtasks((
    [words containing $a a$ exactly once],
    [words in which the first and last letters match],
    [words with alternating letters $a$ and $b$ (eg.: $a b a b a b a$ or $b a b a b$)],
    [words in which every $a$ is followed by $b b$],
    [\*words in which there is at least one $a$ and at least one $b$ between every letter $c$ $Sigma = {a,b,c}$],
    [\* $a^n b^n$ (any number of letter $a$, followed by *the same number of* letter $b$)]
  ))
+ Construct a deterministic finite automaton for each of the following divisibility rules:
  #subtasks((
    [numbers divisible by 5, $Sigma = {0,1,2,...,9}$],
    [numbers divisible by 3, $Sigma = {0,1,2,...,9}$],
    [binary numbers divisible by 2, $Sigma = {0,1}$],
    [\*binary numbers divisible by 3, $Sigma = {0,1}$],
  ), cg:1cm)
